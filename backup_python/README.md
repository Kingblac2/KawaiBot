...
Built using a **Flask** backend with local **Ollama** model inference (`gemma4:latest`) or **Gemini API** (`gemini-1.5-flash`), it demonstrates advanced AI engineering workflows like Chain of Thought (CoT), the ReAct framework, Prompt Chaining, and Structured Output parsing.
...
│   ├── llm.py               # Local Ollama / Gemini API client wrapper
...
If you want to use the **Gemini API** directly, create a `.env` file in the project root:
```text
GEMINI_API_KEY=AIzaSy...
```
If `GEMINI_API_KEY` is not set, the system automatically falls back to your local **Ollama** server.

### 2. Ensure Ollama is running and has gemma4 model (Fallback)
